...
class FloatyExtension {
  ...
  showNotification(message) {
    ...
    const existing = document.getElementById('floaty-popup-notification');
    if (existing) {
      existing.textContent = message;
      // Reset timer if already visible
      clearTimeout(existing._floatyTimeout);
    } else {
      // Create a simple notification
      const notification = document.createElement("div")
      notification.id = 'floaty-popup-notification';
    ...
    // Set or reset the timer for removal
    const notif = document.getElementById('floaty-popup-notification');
    notif._floatyTimeout = setTimeout(() => {
      notif.remove();
    }, 2000); // Show for a longer duration
  }
  ...
  async loadData() {
    try {
      ...
      if (response && response.success && response.data) {
        ...
        this.highlights = (response.data.highlights || []).filter(h => {
